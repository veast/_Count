...
static void doConfirm(resource *r,int N);
...
int getN() {
	...
	int n = 0;
	string temp;
	ReadFile.open("ConfirmConfiguration.txt", ios::in);
	if (ReadFile.fail())
		return 0;
	else {
...
void doConfirm(resource *r, int N) {	//确认过程
	cout << "======================" << endl;
	cout << "下面进行资源确认（输入-1退出; 输入-2清屏并更新资源数目显示）" << endl;
	//输出当前数据1.虚拟机 2.数据库....
	int ch;
...
void doSave(resource *r, int N) {
	ofstream outfile;
	outfile.open("ConfirmConfiguration.txt");
	for (int i = 0; i < N; i++) 
